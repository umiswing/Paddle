...
#define CUDA_ROUTINE_EACH_STREAM_MEMORY_OPERATIONS(__macro) \
  __macro(cuStreamWriteValue32_v2)
CUDA_ROUTINE_EACH_STREAM_MEMORY_OPERATIONS(DECLARE_DYNAMIC_LOAD_CUDA_WRAP);
#endif

/*** cuStreamWriteValue32_v2 seems deprecated after CUDA 12.0, but it is still
available on CUDA 12.3 #if (CUDA_VERSION == 11700 || CUDA_VERSION == 11800)
#define CUDA_ROUTINE_EACH_STREAM_MEMORY_OPERATIONS    \
  __macro(cuStreamWriteValue32_v2);
CUDA_ROUTINE_EACH_STREAM_MEMORY_OPERATIONS(DECLARE_DYNAMIC_LOAD_CUDA_WRAP);
#else
#if CUDA_VERSION >= 800
#define CUDA_ROUTINE_EACH_STREAM_MEMORY_OPERATIONS    \
  __macro(cuStreamWriteValue32);
CUDA_ROUTINE_EACH_STREAM_MEMORY_OPERATIONS(DECLARE_DYNAMIC_LOAD_CUDA_WRAP);
#endif
#endif
***/
